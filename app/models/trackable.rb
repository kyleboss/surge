class Trackable < ActiveRecord::Base
  ...
  INNER JOIN barcode_scans ON trackables.order_id = barcode_scans.order_id
  INNER JOIN rfid_reader_barcode_reader_pairing ON rfid_reader_barcode_reader_pairing.barcode_reader_id = barcode_scans.barcode_reader_id
  WHERE rfid_reader_barcode_reader_pairing.rfid_reader_id = " + rfid_reader_id + " AND
