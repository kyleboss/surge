class RfidScan < ActiveRecord::Base
  ...
  def self.scan(rfid_reader_id, rfid_id)
    ...
    trackable = Trackable.get_new_unpaired_trackables_given_rfid_reader(ruby_rfid_reader.id)
    self.pair_with_rfid(rfid, trackable)
  end

  def self.pair_with_rfid(rfid, trackable)
    if trackable.present?
      RfidTrackablePairing.create({rfid_id: rfid.id, trackable_id: trackable.first.id})
    end
  end
end
