class PatientsController < ApplicationController
  ...
  def get_all_trackables_of_patient()
    ...
    @trackables_of_patient = Trackable.find_by_sql("SELECT t.*, t.id AS trackable_id, a1.*, l.name AS location_name,
      a1.created_at AS last_arrival_time, d.created_at AS last_departure_time, t.patient_id AS patient_id,
      COALESCE(d.created_at, a1.created_at) AS last_update_time,
      CASE WHEN d.created_at IS NULL THEN 'is_arrival' ELSE 'is_departure' END AS update_type
      FROM trackables t
      INNER JOIN arrivals a1 ON (t.id = a1.trackable_id)
      LEFT OUTER JOIN arrivals a2 ON (t.id = a2.trackable_id AND
          (a1.created_at < a2.created_at OR a1.created_at = a2.created_at AND a1.id < a2.id))
      LEFT OUTER JOIN departures d ON (t.id = d.trackable_id AND
          (a1.created_at < d.created_at OR a1.created_at = d.created_at AND a1.id < d.id))
